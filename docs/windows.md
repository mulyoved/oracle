...
- Cookies: the Chrome cookies DB is copied to a temp directory that contains a `Cookies` file to avoid locks; we try native decrypt (DPAPI + AES-GCM) and fall back to `chrome-cookies-secure`. App-bound (`v20`) cookies can still fail decryption; prefer `--browser-manual-login` to reuse a persistent automation profile and sign in once (skips cookie copy entirely). Inline cookies remain available (`--browser-inline-cookies(-file)` / `ORACLE_BROWSER_COOKIES_JSON`).
